...
let _allDayReminderPresetSource;
...
function getEventUpdatedToastMessage(eventEntry, previousEvent) {
    const previousStartTime = previousEvent?.startTime;
    const nextStartTime = eventEntry?.startTime;

    if (previousStartTime && nextStartTime && !previousStartTime.isSameDay(nextStartTime)) {
        const formattedDate = eventEntry.allDay
            ? nextStartTime.toLocaleDateStringJ()
            : nextStartTime.toLocaleStringJ();
        return getMessage("eventRescheduledTo", formattedDate);
    }

    return getMessage("eventUpdated");
}
...
function getEventKey(event) {
    return `${getEventCalendarId(event)}::${event.id}`;
}

function extractTaskIdFromTasksWebViewLink(webViewLink) {
    if (!webViewLink) {
        return;
    }

    const match = webViewLink.match(/tasks\.google\.com\/task\/([^\/?#"'<>\s]+)/i);
    if (match?.[1]) {
        return decodeURIComponent(match[1]);
    }
}

function extractTaskIdFromFocusTimeDescription(description) {
    if (!description) {
        return;
    }

    const match = description.match(/tasks\.google\.com\/task\/([^\/?#"'<>\s]+)/i);
    if (match?.[1]) {
        return decodeURIComponent(match[1]);
    }
}

function deduplicateSharedEvents(events) {
    const eventGroupsById = new Map();

    events.forEach(event => {
        // Skip tasks and snoozer wrappers — they should not be deduplicated
        if (event?.isSnoozer || event?.kind == TASKS_KIND) {
            return;
        }
        const id = event?.id;
        if (!id) {
        ...
        if (!eventGroupsById.has(id)) {
            eventGroupsById.set(id, []);
        }
        eventGroupsById.get(id).push(event);
    });

    // Build a set of event keys to hide (all but the winner in each duplicate group)
    const hiddenEventKeys = new Set();
    const sharedCountByKey = new Map();

    eventGroupsById.forEach(group => {
        if (group.length < 2) {
            return;
        }
        ...
        group.forEach(event => {
            const cal = getEventCalendar(event);
            let score = 0;
            if (cal?.accessRole === "owner") score += 3;
            if (event.organizer?.self) score += 2;
            if (cal?.primary) score += 1;
            if (score > bestScore) {
                bestScore = score;
                winner = event;
            }
        ...
        const winnerKey = getEventKey(winner);
        sharedCountByKey.set(winnerKey, group.length);

        group.forEach(event => {
            const key = getEventKey(event);
            if (key !== winnerKey) {
                hiddenEventKeys.add(key);
            }
        });
    });
    ...
    return events.reduce((deduped, event) => {
        ...
        const key = getEventKey(event);
        if (hiddenEventKeys.has(key)) {
            return deduped;
        }
        if (sharedCountByKey.has(key)) {
            event.sharedCalendarCount = sharedCountByKey.get(key);
        }
        deduped.push(event);
        return deduped;
    }, []);
...
function normalizeLinkedTaskFocusTimeEvents(events) {
    const taskEventsByTaskId = new Map();
    const hiddenEventKeys = new Set();
    const taskOverridesByEventKey = new Map();

    events.forEach(event => {
        if (event?.kind == TASKS_KIND) {
            const taskId = extractTaskIdFromTasksWebViewLink(event.webViewLink);
            if (taskId) {
                taskEventsByTaskId.set(taskId, event);
            }
        }
    });

    events.forEach(event => {
        if (event?.kind == TASKS_KIND || event?.eventType != EventType.FOCUS_TIME || !event.description) {
            return;
        }

        const taskId = extractTaskIdFromFocusTimeDescription(event.description);
        if (!taskId) {
            return;
        }

        const matchingTaskEvent = taskEventsByTaskId.get(taskId);
        if (!matchingTaskEvent || !event.startTime || !event.endTime) {
            return;
        }

        hiddenEventKeys.add(getEventKey(event));

        const timedTaskEvent = deepClone(matchingTaskEvent);
        timedTaskEvent.allDay = false;
        timedTaskEvent.startTime = new Date(event.startTime);
        timedTaskEvent.endTime = new Date(event.endTime);

        taskOverridesByEventKey.set(getEventKey(matchingTaskEvent), timedTaskEvent);
    });

    return events.reduce((normalizedEvents, event) => {
        const eventKey = getEventKey(event);

        if (hiddenEventKeys.has(eventKey)) {
            return normalizedEvents;
        }

        if (taskOverridesByEventKey.has(eventKey)) {
            normalizedEvents.push(taskOverridesByEventKey.get(eventKey));
        } else {
...
async function convertAllEventsToFullCalendarEvents(events) {
    ...
    const normalizedEvents = normalizeLinkedTaskFocusTimeEvents(events);
    const deduplicateSharedEventsSetting = await storage.get("deduplicateSharedEvents");
    const processedEvents = deduplicateSharedEventsSetting ? deduplicateSharedEvents(normalizedEvents) : normalizedEvents;
...
async function openConflictingEventDialog({newEvent, conflictingEvent}) {
    ...
    container.appendChild(document.createTextNode(`(${generateTimeDurationStr({event: conflictingEvent, compact: true})})`));
...
async function setEventDateMessage(eventEntry) {
	...
	if (eventEntry.inputSource == InputSource.QUICK_ADD) {
		var message = await formatEventAddedMessage("<span class='toast-event-title'>" + eventEntry.summary.trim() + "</span>", eventEntry);
        voiceMessage = await formatEventAddedMessage(eventEntry.summary.trim(), eventEntry);
    ...
    if (await storage.get("notificationVoice") && voiceMessage) {
        ChromeTTS();
        await ChromeTTS.queue(voiceMessage);
    }
}
...
function getCreateEventCollapsibleDefaultSelectionWrappers($createEventDialog) {
    return [
        $createEventDialog.querySelector("#createEventCalendar-wrapper"),
        $createEventDialog.querySelector("#event-reminders-wrapper"),
        $createEventDialog.querySelector("#event-transparency-and-visibility-wrapper")
    ].filter(Boolean);
}

function updateCreateEventDefaultSelectionsSummary($createEventDialog = byId("createEventDialog")) {
    if (!$createEventDialog || $createEventDialog.dataset.collapsibleDefaults != "true") {
        return;
    }

    const $calendarDropdown = $createEventDialog.querySelector("#createEventCalendarsMenu");
    const $transparencyDropdown = $createEventDialog.querySelector("#event-transparency");
    const $visibilityDropdown = $createEventDialog.querySelector("#event-visibility");
    const remindersCount = $createEventDialog.querySelectorAll("#event-reminders .calendarReminder").length;
    const calendarName = $calendarDropdown?.selectedOptions?.[0]?.textContent?.trim() || "";
    const calendarDetails = [
        $transparencyDropdown?.selectedOptions?.[0]?.textContent?.trim(),
        $visibilityDropdown?.selectedOptions?.[0]?.textContent?.trim(),
        `${remindersCount} ${getMessage("notifications")}`
    ].filter(Boolean).join(" • ");

    const $calendarText = $createEventDialog.querySelector("#event-default-selections-summary-calendar-text");
    const $detailsText = $createEventDialog.querySelector("#event-default-selections-summary-details-line");
    const $calendarDot = $createEventDialog.querySelector("#event-default-selections-summary-calendar-dot");

    if ($calendarText) {
        $calendarText.textContent = calendarName;
    }

    if ($detailsText) {
        $detailsText.textContent = calendarDetails;
    }

    if ($calendarDot) {
        let calendarColor;
        const calendarId = $calendarDropdown?.value;
        const selectedCalendar = calendarId ? getCalendarById(calendarId) : null;

        if (selectedCalendar?.colorId && colors?.calendar?.[selectedCalendar.colorId]?.background) {
            calendarColor = colors.calendar[selectedCalendar.colorId].background;
        } else if (selectedCalendar?.backgroundColor) {
            calendarColor = selectedCalendar.backgroundColor;
        }

        $calendarDot.style.background = calendarColor || "transparent";
    }
}

function applyCreateEventDefaultSelectionsCollapsedState($createEventDialog = byId("createEventDialog")) {
    if (!$createEventDialog || $createEventDialog.dataset.collapsibleDefaults != "true") {
        return;
    }

    const $summaryWrapper = $createEventDialog.querySelector("#event-default-selections-summary-wrapper");
    const taskSelected = $createEventDialog.classList.contains("task-selected");
    const collapsed = $createEventDialog.dataset.defaultSelectionsCollapsed == "true";
    const shouldCollapse = !taskSelected && collapsed;

    getCreateEventCollapsibleDefaultSelectionWrappers($createEventDialog).forEach($wrapper => {
        $wrapper.toggleAttribute("hidden", shouldCollapse);
    });

    if ($summaryWrapper) {
        $summaryWrapper.toggleAttribute("hidden", !shouldCollapse);
    }
}

function animateDialogResize($dialog, mutate) {
    if (!$dialog || typeof mutate !== "function") {
        return;
    }
    ...
    const $message = $dialog.querySelector("#native-dialog-message");
    const firstDialogHeight = $dialog.offsetHeight;
    const firstMessageHeight = $message ? $message.offsetHeight : 0;

    mutate();
    const lastDialogHeight = $dialog.offsetHeight;
    const lastMessageHeight = $message ? $message.offsetHeight : 0;

    if (!firstDialogHeight || !lastDialogHeight || firstDialogHeight == lastDialogHeight) {
        return;
    }
    ...
    $dialog.style.height = firstDialogHeight + "px";

    if ($message && firstMessageHeight && lastMessageHeight && firstMessageHeight != lastMessageHeight) {
        $message.style.overflow = "hidden";
        $message.style.height = firstMessageHeight + "px";
    }

    // Force the browser to commit start heights before starting the animation.
    void $dialog.offsetHeight;

    const dialogAnimation = $dialog.animate([
        { height: firstDialogHeight + "px" },
        { height: lastDialogHeight + "px" }
    ], {
        duration: 220,
    ...
    if ($message && firstMessageHeight && lastMessageHeight && firstMessageHeight != lastMessageHeight) {
        messageAnimation = $message.animate([
            { height: firstMessageHeight + "px" },
            { height: lastMessageHeight + "px" }
        ], {
            duration: 220,
            easing: "cubic-bezier(.2,.9,.2,1)",
            fill: "forwards"
    ...
    Promise.all([
        ...
        messageAnimation ? messageAnimation.finished.catch(() => {}) : Promise.resolve()
    ]).finally(cleanup);
}

function setCreateEventReminderPresetValue($calendarReminder, allDay) {
    const $reminderPreset = $calendarReminder.querySelector(".reminderPreset");
    if (!$reminderPreset) {
        return;
    }

    const reminderMethod = $calendarReminder.querySelector(".reminderMethod")?.value || "popup";
    const $reminderMinutes = $calendarReminder.querySelector(".reminderMinutes");
    const presetValues = getReminderPresetValues(allDay, $calendarReminder._presetReminders).map(String);
    const reminderMinutes = String(parseInt($reminderMinutes.value));

    Array.from($reminderPreset.options).forEach(option => {
        if (option.value != "custom") {
            const minutes = parseInt(option.value);
            if (!Number.isNaN(minutes)) {
                if (presetValues.includes(option.value)) {
                    if (option.value == reminderMinutes) {
                        option.dataset.method = reminderMethod;
                    } else if (!allDay) {
                        option.dataset.method = "popup";
                    }
                }
                const labelMethod = presetValues.includes(option.value) ? (option.dataset.method || "popup") : reminderMethod;
                option.textContent = getReminderPresetLabel(minutes, allDay, labelMethod);
            }
        }
    });

    if (presetValues.includes(reminderMinutes)) {
        $reminderPreset.value = reminderMinutes;
    } else {
        const existingOption = Array.from($reminderPreset.options).find(option => option.value == reminderMinutes);

        if (!existingOption) {
            const customOption = Array.from($reminderPreset.options).find(option => option.value == "custom");
            const option = document.createElement("option");
            option.value = reminderMinutes;
            option.textContent = getReminderPresetLabel(parseInt(reminderMinutes), allDay, reminderMethod);

            if (customOption) {
                customOption.before(option);
            } else {
                $reminderPreset.appendChild(option);
            }
        } else {
            if (!allDay && presetValues.includes(reminderMinutes)) {
                existingOption.dataset.method = reminderMethod;
            }
            const labelMethod = presetValues.includes(reminderMinutes) ? (existingOption.dataset.method || "popup") : reminderMethod;
            existingOption.textContent = getReminderPresetLabel(parseInt(reminderMinutes), allDay, labelMethod);
        }

        $reminderPreset.value = reminderMinutes;

        sortRemindersOptions($calendarReminder);
    }
}

async function openCreateEventReminderCustomDialog($createEventDialog, $calendarReminder, allDay) {
    const reminderEditor = generateReminderLine(generateReminderFromValueAndPeriod($calendarReminder));
    reminderEditor.classList.add("custom-reminder-editor");
    reminderEditor.querySelector(".deleteReminder")?.remove();

    let saved = false;
    let syncReminderMinutes = () => {};

    openDialog(reminderEditor, {
        title: getMessage("custom"),
        cancel: true,
        buttons: [{
            ...
            onClick: (dialog) => {
                ...
                $calendarReminder.querySelector(".reminderMethod").value = reminderEditor.querySelector(".reminderMethod").value;
                $calendarReminder.querySelector(".reminderMinutes").value = reminderEditor.querySelector(".reminderMinutes").value;
                $calendarReminder.querySelector(".lastUpdated").value = reminderEditor.querySelector(".lastUpdated").value;

                setCreateEventReminderPresetValue($calendarReminder, allDay);

                createEventRemindersChanged = true;
                updateCreateEventDefaultSelectionsSummary($createEventDialog);
                saved = true;
                dialog.close();
            }
        }]
    ...
    requestAnimationFrame(() => {
        if (!reminderEditor.isConnected) {
            return;
        }

        const $reminderMinutes = reminderEditor.querySelector(".reminderMinutes");
        const $reminderValuePerPeriod = reminderEditor.querySelector(".reminderValuePerPeriod");
        const $reminderPeriod = reminderEditor.querySelector(".reminderPeriod");
        let $beforeTime;

        if (allDay) {
            const displayState = getAllDayReminderDisplayState($reminderMinutes.value);
            $reminderMinutes.value = displayState.reminderMinutes;

            const beforeAtText = document.createElement("span");
            beforeAtText.className = "beforeAtText";
            beforeAtText.textContent = getMessage("beforeStart");

            $beforeTime = createBeforeAtTimeDropdown(minutesToTimeValue(displayState.beforeTimeMinutes));

            const $lastUpdated = reminderEditor.querySelector(".lastUpdated");
            reminderEditor.insertBefore(beforeAtText, $lastUpdated);
            reminderEditor.insertBefore($beforeTime, $lastUpdated);
        }

        initReminderPeriod($reminderValuePerPeriod, $reminderPeriod, $reminderMinutes, allDay, allDay);

        function enforceAllDayZeroDaysBeforeTimeRule() {
            if (!allDay || !$beforeTime) {
                return;
            }

            const reminderValue = parseInt($reminderValuePerPeriod.value) || 0;
            const isZeroDays = ($reminderPeriod.value == "days" || $reminderPeriod.value == "weeks") && reminderValue == 0;

            if (isZeroDays) {
                $beforeTime.value = "0:00";
                $beforeTime.setAttribute("disabled", "true");
            } else {
                $beforeTime.removeAttribute("disabled");
            }
        }

        enforceAllDayZeroDaysBeforeTimeRule();
        ...
        syncReminderMinutes = () => {
            ...
            if (allDay && $beforeTime) {
                const beforeTimeParts = ($beforeTime.value || "0:00").split(":");
                const beforeHour = parseInt(beforeTimeParts[0]) || 0;
                const beforeMinute = parseInt(beforeTimeParts[1]) || 0;
                const beforeTimeInMinutes = (beforeHour * HOUR_IN_MINUTES) + beforeMinute;

                $reminderMinutes.value = (parseInt($reminderMinutes.value) || 0) - beforeTimeInMinutes;
            }
        };

        replaceEventListeners(reminderEditor.querySelectorAll("select"), "change", function() {
            syncReminderMinutes();
        });

        replaceEventListeners(reminderEditor.querySelectorAll("j-input"), "change", function() {
            syncReminderMinutes();
        });
    });
...
function initCreateEventReminderLine($createEventDialog, $calendarReminder, allDay) {
	const $eventReminders = $createEventDialog.querySelector("#event-reminders");
	const $reminderPreset = $calendarReminder.querySelector(".reminderPreset");
	const $reminderMethod = $calendarReminder.querySelector(".reminderMethod");
	const $reminderMinutes = $calendarReminder.querySelector(".reminderMinutes");

	setCreateEventReminderPresetValue($calendarReminder, allDay);

	replaceEventListeners($reminderPreset, "click", function() {
		this.dataset.previousValue = this.value;
	});

	replaceEventListeners($reminderPreset, "change", async function() {
		if (this.value == "custom") {
			const previousValue = this.dataset.previousValue || this.value;
			const saved = await openCreateEventReminderCustomDialog($createEventDialog, $calendarReminder, allDay);
			if (!saved) {
        ...
        $reminderMethod.value = this.selectedOptions?.[0]?.dataset?.method || "popup";
		$reminderMinutes.value = this.value;
		createEventRemindersChanged = true;
        updateCreateEventDefaultSelectionsSummary($createEventDialog);
	});
...
async function getAllDayReminderDefaults(calendar, event, editing) {
    let allDayReminders;

    if (editing && !event.extendedProperties?.private?.useDefaultAllDayReminders && event?.allDay && event.reminders && !event.reminders.useDefault && event.reminders.overrides?.length) {
        allDayReminders = deepClone(event.reminders.overrides);
    } else {
        const cachedFeedsDetails = await storage.get("cachedFeedsDetails");
        const allDayDefaultReminders = cachedFeedsDetails[calendar?.id].allDayDefaultReminders;
        if (Array.isArray(allDayDefaultReminders) && allDayDefaultReminders.length) {
            allDayReminders = deepClone(allDayDefaultReminders);
        } else if (!editing && calendar?.defaultReminders?.length) {
            allDayReminders = [getDefaultAllDayReminder()];
        } else {
            allDayReminders = [];
        }
    }
    ...
    const reminderMinutesAdded = new Set();

    const parsedDefaultEventNotificationTime = getDefaultEventNotificationTime();
    const beforeTimeInMinutes = (parsedDefaultEventNotificationTime.getHours() * HOUR_IN_MINUTES) + parsedDefaultEventNotificationTime.getMinutes();

    function addReminders(reminders) {
        reminders?.forEach(reminder => {
            const reminderMinutes = parseInt(reminder.minutes);
            const reminderKey = `${reminder.method}:${reminderMinutes}`;
            if (!reminderMinutesAdded.has(reminderKey)) {
                reminderMinutesAdded.add(reminderKey);
    ...
    // Always include baseline all-day options.
    const baselineAllDayRemindersInMinutes = [DAY_IN_MINUTES, (2 * DAY_IN_MINUTES), WEEK_IN_MINUTES];
    const baselineReminders = baselineAllDayRemindersInMinutes.map(minutes => ({
        method: "popup",
        minutes: minutes - beforeTimeInMinutes,
    }));

    addReminders(baselineReminders);
    addReminders(allDayReminders);

    if (editing && event?.allDay && event.reminders?.overrides?.length) {
        addReminders(event.reminders.overrides);
    }

    _allDayReminderPresetSource = mergedReminders.sort((a, b) => a.minutes - b.minutes);
...
function initAllReminders($createEventDialog, allDay) {
    ...
    $eventReminders.querySelectorAll(".calendarReminder").forEach($calendarReminder => {
        initCreateEventReminderLine($createEventDialog, $calendarReminder, allDay);
    });
    ...
    onClickReplace($addReminder, function () {
        let reminder;
        if (allDay) {
            reminder = getDefaultAllDayReminderAdd();
        } else {
            reminder = getDefaultTimedReminderAdd();
        }

        const presetReminders = allDay ? (_allDayReminderPresetSource || []) : null;
        const reminderLine = generateCreateEventReminderLine(reminder, allDay, presetReminders);

        $addReminder.before(reminderLine);
        createEventRemindersChanged = true;

        initCreateEventReminderLine($createEventDialog, reminderLine, allDay);
        $createEventDialog.querySelector("#native-dialog-message").scrollTop += 40;
        updateCreateEventDefaultSelectionsSummary($createEventDialog);
    });
}

function sortRemindersOptions($parentNode) {
    // sorting the options after, because I add some using generateCreateEventReminderLine and then I add some in initAllReminders > setCreateEventReminderPresetValue
    $parentNode.querySelectorAll(".reminderPreset").forEach($reminderPreset => {
        const options = Array.from($reminderPreset.options);
        options.sort((a, b) => {
            const aValue = parseInt(a.value);
            const bValue = parseInt(b.value);

            const aIsNumber = !Number.isNaN(aValue);
            const bIsNumber = !Number.isNaN(bValue);
...
function changeReminders(allDay, $createEventDialog, allDayReminders, timedReminders) {
	...
	if (allDay) {
		allDayReminders.forEach(reminder => {
            $eventReminders.querySelector(".addReminder").before(generateCreateEventReminderLine(reminder, true, _allDayReminderPresetSource));
		});
		initAllReminders($createEventDialog, true);
		createEventAllDay = true;
	} else {
        //timedReminders.sort((a, b) => parseInt(a.minutes) - parseInt(b.minutes));
		timedReminders.forEach(reminder => {
            $eventReminders.querySelector(".addReminder").before(generateCreateEventReminderLine(reminder, false));
		});
		initAllReminders($createEventDialog, false);
        sortRemindersOptions($eventReminders);
...
async function showCreateBubble(params) {
    ...
    function setReminders(eventEntry) {
        if (createEventRemindersChanged) {
            eventEntry.reminders = generateRemindersForEventEntry();

            if (params.editing && eventEntry.extendedProperties?.private?.useDefaultAllDayReminders) {
                eventEntry.extendedProperties ||= {};
                eventEntry.extendedProperties.private ||= {};
                eventEntry.extendedProperties.private.useDefaultAllDayReminders = null;
            }
        } else if (eventEntry.allDay) {
            eventEntry.reminders = generateRemindersForEventEntry();

            eventEntry.extendedProperties ||= {};
            eventEntry.extendedProperties.private ||= {};
            eventEntry.extendedProperties.private.useDefaultAllDayReminders = true;
            /*
            eventEntry.reminders = {
                useDefault: false,
            };
            */
        }
	...
	if (params.editing) {
        ...
        allDayReminders = await getAllDayReminderDefaults(calendar, event, params.editing);
	} else {
        if (event.allDay && !await storage.get("selectAllDay")) {
            ...
            if (event.startTime.isToday()) {
                ...
                event.startTime.setHours(await getDefaultHourForTimedEvent());
            }
        }
	...
	function initEventRemindersNode() {
        let reminders;
        if (params.autoRestore && params.event.reminders?.overrides) {
            reminders = params.event.reminders.overrides;
        } else {
            if (params.editing) {
                if (!event.reminders || event.reminders.useDefault || (event.allDay && (!event.reminders.overrides || event.extendedProperties?.private?.useDefaultAllDayReminders))) {
                    if (event.allDay) {
        ...
        const presetReminders = event.allDay ? _allDayReminderPresetSource : null;
        emptyAppend(eventReminders, generateCreateEventReminderSection(reminders, event.allDay, presetReminders));
	}
    ...
    byId("createEventCalendarsMenu").addEventListener("change", async function(e) {
    ...
    const eventTimeZone = event.timeZone || event.start?.timeZone || event.end?.timeZone;
    const primaryTimezone = await getPrimaryTimezone();
    const editingCustomTimezone = params.editing && eventTimeZone && eventTimeZone != primaryTimezone;

    if (editingCustomTimezone) {
        // Convert to the event's timezone so the user edits the same wall-clock times.
        eventStartTime = convertTimeToTimezone(new Date(event.startTime), eventTimeZone);
        eventEndTime = convertTimeToTimezone(new Date(eventEndTime), eventTimeZone);
    } else {
        eventStartTime = new Date(event.startTime);
    }

	const deltaDays = Math.round(eventEndTime.diffInDays(eventStartTime));
    let deltaMinutes = eventEndTime.diffInMinutes(eventStartTime);
    ...
    eventStartDatePicker.dateTime = eventStartTime;
	...
	//$("#eventStartDate").datepicker("setDate", eventStartTime);
    ...
    if (event.allDay) {
        ...
        eventStartTimePicker.dateTime = eventStartTime;
    }
    ...
    if (!eventEndTime || eventStartTime.isSameDay(eventEndTime)) {
    ...
    replaceEventListeners("#eventAllDay", "change", async function () {
        if (params.editing) {
        ...
        if (event.allDay) {
            ...
            if (!params.editing && !eventStartTime.isToday()) {
                const time = new DateZeroTime();
                time.setHours(await getDefaultHourForTimedEvent());
        ...
        if (!params.editing) {
            ...
            updateCreateEventDefaultSelectionsSummary(byId("createEventDialog"));
        }
    ...
    if (editingCustomTimezone) {
        ...
        show($dropdownWrapper);
        await generateTimezoneDropdown("timezone-dropdown-wrapper", true);
        $dropdownWrapper.value = eventTimeZone;
    } else {
    ...
    function initDetectTimeAndReminders({ showDetectTime, skipReminders, skipRecurringInit, userGesture } = {}) {
        ...
        if (params.autoRestore) {
            ...
            // Keep baseline in sync for restored edits so unchanged recurrence isn't treated as modified.
            if (params.editing) {
                _recurrenceDropdownValue = recurrenceDropdownValue ?? "";
            }
        ...
        function setMonthlyOnDayPaperItem(selectedValue, message) {
            ...
            const dayName = new Intl.DateTimeFormat(locale, { weekday: 'long' }).format(eventStartTime);
        ...
        function initRepeatOnWrapper(value, byday, byMonthDay, bysetpos) {
            if (!value || value == "WEEKLY") {
                ...
                if (value == "MONTHLY") {
                    ...
                    if (byMonthDay == -1 || isLastDayOfMonth(eventStartTime)) {
                    ...
                    if (!byMonthDay) {
                        byMonthDay = eventStartTime.getDate();
                    }
                    ...
                    if (bysetpos == 1 || eventStartTime.getDate() <= 7) {
                        setMonthlyOnDayPaperItem(1, "monthlyOnTheFirstX");
                    } else if (bysetpos == 2 || eventStartTime.getDate() <= 14) {
                        setMonthlyOnDayPaperItem(2, "monthlyOnTheSecondX");
                    } else if (bysetpos == 3 || eventStartTime.getDate() <= 21) {
                        setMonthlyOnDayPaperItem(3, "monthlyOnTheThirdX");
                    }
                    
                    if (bysetpos == 4 || (21 < eventStartTime.getDate() && eventStartTime.getDate() <= 28)) {
                        setMonthlyOnDayPaperItem(4, "monthlyOnTheFourthX");
                    }
                    if (bysetpos == -1 || eventStartTime.getDate() >= 25) {
            ...
            if (value == "custom") {
                ...
                $dialogContent.querySelectorAll("#repeatOnDays j-button").forEach(($button, index) => {
                    ...
                    if (existingRule) {
                        ...
                        if (eventStartTime.getDay() === index) {
                            $button.classList.add("active-state");
                        }
                    }
                });
                ...
                repeatEndsOnDatePicker.dateTime = eventStartTime.addDays(90);
                            ...
                            onClick: (dialog) => {
                                ...
                                if (frequency == "MONTHLY") {
                                    if (monthyOn == -1) {
                                        ...
                                        rrule += `;BYMONTHDAY=${eventStartTime.getDate()}`;
                                    } else {
    ...
    // note had to use event.endTime (not eventEndTime) because we force set it for timezone editing reasons
    const spansMultipeDays = event.allDay && eventStartTime && event.endTime && !eventStartTime.isSameDay(eventEndTime);
	...
	function initEventEntry() {
        ...
        if (!params.editing && isVisible("#detectTime") && doesEventTitleHaveTime()) {
			eventEntry = new EventEntry();
			eventEntry.startTime = eventStartTime;
		} else {
        ...
        const previousRecurrenceDropdownValue = params.autoRestore
            ? (params.recurringEventDetails?.dropdownValue ?? _recurrenceDropdownValue ?? "")
            : (_recurrenceDropdownValue ?? "");
                            ...
                            if (editing) {
                                ...
                                if (byId("eventAllDay")._originalAllDayFlag == eventEntry.allDay) {
                                    if (eventEntry.allDay) {
                                        if (eventStartTime.isSameDay(eventEntry.startTime) && eventEndTime.isSameDay(eventEntry.endTime)) {
                                            timeChanged = false;
                                        }
                                    } else {
                                        if (eventStartTime.isEqual(eventEntry.startTime) && eventEndTime.isEqual(eventEntry.endTime)) {
                                            timeChanged = false;
                                        }
                                    }
                                }
                                ...
                                const previousEventSnapshot = deepClone(event);
                                try {
                                    const response = await updateEvent(updateEventParams);
                                    if (response.cancel) {
                                        dialog.classList.remove("hide-temporarily");
                                        ...
                                        showToast(getEventUpdatedToastMessage(eventEntry, previousEventSnapshot));
    ...
    _allDayReminderPresetSource ||= allDayReminders;

    if (!params.editing) {
        $createEventDialog.dataset.collapsibleDefaults = "true";
        $createEventDialog.dataset.defaultSelectionsCollapsed = "true";

        onClickReplace($createEventDialog.querySelector("#event-default-selections-summary"), function() {
            animateDialogResize($createEventDialog, () => {
                $createEventDialog.dataset.defaultSelectionsCollapsed = "false";
                applyCreateEventDefaultSelectionsCollapsedState($createEventDialog);
            });
        });

        replaceEventListeners($createEventDialog.querySelectorAll("#event-transparency, #event-visibility"), "change", function() {
            updateCreateEventDefaultSelectionsSummary($createEventDialog);
        });

        const $eventLocation = byId("eventLocation");
        const $eventLocationReveal = byId("eventLocationReveal");
        const $eventLocationWrapper = $eventLocation?.parentElement;
        const $currentPage = byId("currentPage");
        const $eventDescription = byId("eventDescription");
        const $eventDescriptionReveal = byId("eventDescriptionReveal");

        if ($eventLocation && $eventLocationWrapper && $eventLocationReveal && !params.autoRestore) {
            hide($eventLocationWrapper);
            show($eventLocationReveal);

            onClickReplace($eventLocationReveal, function() {
                hide($eventLocationReveal);
                console.log("aa-show")
                show($eventLocationWrapper);
                $eventLocation.focus();
            });
        }

        if ($eventDescription && $eventDescriptionReveal && !params.autoRestore) {
            hide($eventDescription);
            show($eventDescriptionReveal);

            onClickReplace($eventDescriptionReveal, function() {
                hide($eventDescriptionReveal);
                show($eventDescription);
                $eventDescription.focus();
            });
        }
    } else {
        $createEventDialog.dataset.collapsibleDefaults = "false";
    }
    ...
    function selectEvent() {
        ...
        initAllDay(event.allDay);
        applyCreateEventDefaultSelectionsCollapsedState(byId("createEventDialog"));
    }
    ...
    setSafeHTML($createEventDialog.querySelector("#eventDescription"), htmlDescription);
    ...
    const $currentPage = byId("currentPage");
    function setCurrentPageGenericLinkIcon() {
        $currentPage.removeAttribute("src");
    ...
    if (tab?.favIconUrl) {
        ...
        const img = new Image();
        loadImage(img).then(() => {
            $currentPage.removeAttribute("icon");
            $currentPage.setAttribute("src", tab.favIconUrl);
        }).catch(() => {
            setCurrentPageGenericLinkIcon();
        });
    ...
    if (tab) {
        onClickReplace($currentPage, async () => {
            hide($currentPage);
            byId("eventLocationReveal").click();
            ...
            if (url != response.description) {
                setSafeHTML(byId("eventDescription"), response.description);
...
async function searchEvents() {
    ...
    const promises = calendarIdsToSearch.map(calendarId => {
        return oauthDeviceSend({
            ...
            data: {
                q: searchStr,
                eventTypes: [EventType.DEFAULT, EventType.BIRTHDAY, EventType.FROM_GMAIL],
                singleEvents: true,
                orderBy: "startTime",
                maxResults: 1000
            }
...
async function displayAgenda(params = {}) {
    ...
    eventsToDisplay = normalizeLinkedTaskFocusTimeEvents(eventsToDisplay);
    ...
    function displayAgendaEvents(eventParams) {
        ...
        events.forEach((event, index) => {
            ...
            if (params.gotoDate && event.startTime.isBefore(params.start)) {
                ...
                const isTaskBeforeToday = event.kind == TASKS_KIND && event.startTime.isBefore(today());
    			// optimize for speed by skipping over before today events
                if (params.showStartingToday && (event.endTime?.isEqualOrBefore(today()) || (multiDayEvent && event.startTime.isBefore(today())) || isTaskBeforeToday)) { // (event.endTime?.isEqualOrBefore(today()) || multiDayEvent)
	...
	if (params.prepend) {
        ...
        if (scrollTarget) {
            const addedHeight = scrollTarget.scrollHeight - previousScrollHeight;
            scrollTarget.scrollTop = previousScrollTop + Math.max(0, addedHeight);
        }
	} else {
...
async function changeCalendarView(viewName) {
	...
	//if (viewName != previousCalendarView) {
		...
		if ((previousCalendarView == CalendarView.AGENDA && viewName != CalendarView.AGENDA) || previousCalendarView == CalendarView.LIST_WEEK || viewName == CalendarView.LIST_WEEK) {
			location.reload();
        } else if (viewName == CalendarView.AGENDA) {
            location.reload();
		} else {
...
async function initOptionsMenu() {
    ...
    } else if (calendarView == CalendarView.MONTH_SCROLL) {
        byId("viewMonthScroll").classList.add("selected");
...
async function showDetailsBubble(params) {
        ...
        editEventButton = {
            ...
            onClick: async (dialog) => {
                ...
                if (isSnoozer) {
                    ...
                    const RESTRICTED_EVENT_TYPES = [EventType.BIRTHDAY, EventType.FOCUS_TIME, EventType.FROM_GMAIL, EventType.OUT_OF_OFFICE, EventType.WORKING_LOCATION];
                    const isRestrictedEventType = RESTRICTED_EVENT_TYPES.includes(event.eventType);
                    if (isRestrictedEventType) {
                        // There's an Google Calandar API issue with modifiing these events always returuning 400 bad request ref: https://issuetracker.google.com/issues/359257537
                        // So just opening edit link google calendar ui instead
                        try {
                            const u = new URL(event.htmlLink);
                            const eid = u.searchParams.get("eid");
                            const editUrl = `https://calendar.google.com/calendar/r/eventedit/${encodeURIComponent(eid)}`;
                            openUrl(setUrlParam(editUrl, "authuser", email));
                        } catch {
                            openUrl(getEventUrl(event));
                        }
                    } else {
    ...
    if (event.conferenceData?.createRequest?.status?.statusCode == "pending") {
        ...
        if (videoMeetingDetails.conferenceVideo) {
            ...
            setSafeHTML(byId("clickedEventVideoLabel"), label);
        ...
        if (phone) {
            ...
            setSafeHTML(byId("clickedEventPhoneLabel"), label);
        } else {
            hide("#clickedEventPhoneWrapper");
        }
    ...
    if (event.conferenceData?.notes) {
        setSafeHTML($dialog.querySelector("#clickedEventVideoNotes"), event.conferenceData?.notes);
	...
	if (event.attendees) {
        ...
        if (showGoingButtons) {
            ...
            goingYesNoMaybeButtons.push({
                ...
                classList: goingYes ? ["going-button", "colored"] : ["going-button", "outline"],
                onClick: () => {
            ...
            goingYesNoMaybeButtons.push({
                id: goingNoButtonId,
                label: getMessage("no"),
                classList: goingNo ? ["going-button", "colored"] : ["going-button", "outline"],
                onClick: () => {
            ...
            goingYesNoMaybeButtons.push({
                ...
                classList: goingMaybe ? ["going-button", "colored"] : ["going-button", "outline"],
                onClick: () => {
    ...
    if (event.kind == TASKS_KIND) {
        ...
        if (taskList) {
            ...
            $desc.textContent = taskList.title;
    ...
    const response = await generateReminderTimes(event);
    ...
    $openInCalendarButton.addEventListener("click", function() {
		...
		} else if (event.webViewLink) {
            openUrl(event.webViewLink);
        } else {
            openEventUrl(event);
		}
	}, { signal });
...
function patchToResizePopupThatHasAStubbornLargeWidth() {
    const width = parseFloat(getComputedStyle(htmlElement).width);
    if (width >= 700) {
...
async function init() {
    ...
    sideRailMenu.addEventListener("beforetoggle", async function(e) {
        if (e.newState == "open") {
            ...
            // v2 commented this loaded because on 2nd opening of side rail the calendar would be squeezed - other solution was to .render() with a delay
    ...
    if (typeof loggedOut === "undefined" || loggedOut) { // !loggedOut  - commented because issue occured when invalid creditials
        ...
        //docReady().then(async () => {
            if (calendarView == CalendarView.AGENDA) {
                ...
                patchToResizePopupThatHasAStubbornLargeWidth();
                ...
                if (calendarView == CalendarView.MONTH_SCROLL) {
                    views.multiMonth = {
                        duration: MULTI_MONTH_DURATION
                    };
                }
                ...
                const fullCalendarParams = {
                    locale: locale,
                    views: views,
                    initialView: getFCViewName(calendarView), // "dayGridYear"
                    ...
                    viewDidMount: function(info) {
                        ...
                        } else if (info.view.type == getFCViewName(CalendarView.MONTH_SCROLL)) {
                            if (scroller) {
                                replaceEventListeners(scroller, "scroll", function(e) {
                                    if (!globalThis.multiMonthScrolled) {
                                        globalThis.multiMonthScrolled = true;
                                        fullCalendar.refetchEvents();
                                    }
                                });
                            }
                    ...
                    eventDidMount: function(info) {
                        ...
                        if (jEvent?.sharedCalendarCount > 1) {
                            const sharedIcon = document.createElement("span");
                            sharedIcon.classList.add("shared-calendar-icon");
                            if (eventTitleNode) {
                                eventTitleNode.prepend(sharedIcon);
                            }
                            title = `${title} (in ${jEvent.sharedCalendarCount} calendars)`;
                        }
                            ...
                            if (isCtrlPressed(info.jsEvent)) {
                                ...
                                ensureSendNotificationDialog({ event: jEvent, action: SendNotificationsAction.EDIT }).then(response => {
                                    if (response.cancel) {
                                        ...
                                        const previousEventSnapshot = deepClone(jEvent);
                ...
                if (calendarView == CalendarView.MONTH_SCROLL) {
                    fullCalendarParams.multiMonthMaxColumns = 1;
                }
                ...
                fullCalendarSource = {
                    ...
                    events: function(fetchInfo, successCallback, failureCallback) {
                        ...
                        getEventsWrapper().then(async events => {
                            ...
                            const calendarView = await getCalendarView();
                            const firstTimeLoadingYear = (calendarView == CalendarView.MONTH_SCROLL || calendarView == CalendarView.YEAR) && !globalThis.yearViewPartTwo;
                                    ...
                                    if (firstTimeLoadingYear) {
                                        ...
                                        requestIdleCallback(() => {
                                            getEventsWrapper().then(async events => {
                                                ...
                                                let loadMoreStart = await getEndDateAfterThisMonth();
                                                let loadMoreEnd;
                                                if (calendarView == CalendarView.MONTH_SCROLL) {
                                                    loadMoreEnd = end;
                                                } else {
                                                    loadMoreEnd = new Date(loadMoreStart.getFullYear(), 11, 31, 23, 59, 59, 999);
                                                }
                    
                                                console.log("requested start/stop: " + events.length + " start: " + loadMoreStart + " end: " + loadMoreEnd, fetchInfo);

                                                fetchEvents(events, loadMoreStart, loadMoreEnd).then(async allEvents => {
                                                    if (allEvents) {
                                                        console.time("addEvent");
                                                        const fcEvents = await convertAllEventsToFullCalendarEvents(allEvents);
                                                        fullCalendar.batchRendering(() => {
                                                    ...
                                                    if (calendarView == CalendarView.YEAR) {
                                                        ...
                                                        loadMoreStart = new Date(todayDate.setMonth(todayDate.getMonth()-2));
                                                        loadMoreStart.setDate(1);
                                                        loadMoreEnd = getStartDateBeforeThisMonth();
                                                        fetchEvents(events, loadMoreStart, loadMoreEnd).then(async allEvents => {
                                                            if (allEvents) {
                                                                console.time("addEvent");
                                                                const fcEvents = await convertAllEventsToFullCalendarEvents(allEvents);
                                                            ...
                                                            globalThis.yearViewPartTwoBeforeTodayDetails = {
                                                                start: new Date(loadMoreStart.getFullYear(), 0, 1, 0, 0, 0, 0),
                                                                end: loadMoreStart,
                                                                events: events
                                                            };
    ...
    docReady().then(async () => {
        ...
        if (await shouldShowExtraFeature()) {
            ...
            Controller.getMinimumPayment().then(minPaymentObj => {
                const reducedDonationPopupLine1 = getMessage("reducedDonationAd_popup_line1");
                const reducedDonationPopupLine2 = getMessage("reducedDonationAd_popup_line2", [getMessage("extraFeatures"), formatCurrency(minPaymentObj.getOneTimeReducedPayment())]);

                setSafeHTML($newsNotificationReducedDonationMessage, `${reducedDonationPopupLine1}<br>${reducedDonationPopupLine2}`);
                show($newsNotificationReducedDonationMessage);
            });
    ...
    onClick("#mainOptions", async () => {
        ...
        onClick("#viewMonthScroll", function() {
            fullCalendar.setOption("multiMonthMaxColumns", 1);
            fullCalendar.setOption("duration", MULTI_MONTH_DURATION);
            fullCalendar?.refetchEvents();
            changeCalendarView(CalendarView.MONTH_SCROLL);
            closeMenu(this);
        });

        onClick("#viewYear", function() {
            if (globalThis.fullCalendar) {
                fullCalendar.setOption("multiMonthMaxColumns", 3);
                fullCalendar.setOption("duration", { months: 12 });
                fullCalendar?.refetchEvents();
            }
            changeCalendarView(CalendarView.YEAR);
            closeMenu(this);
