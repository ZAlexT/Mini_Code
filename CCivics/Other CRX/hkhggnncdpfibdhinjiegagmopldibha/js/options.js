		...
		if (tabName == "welcome") {
			...
			addEventListeners("#showContextMenuItem", "change", function (e) {
				if (this.checked) {
                    ...
                    show("#contextMenuFirstHourOfDayWrapper");
                    sendMessageToBG("addChangeContextMenuItems");
				} else {
                    hide("#showOnlyQuickWhenTextSelected");
                    hide("#contextMenuFirstHourOfDayWrapper");
            ...
            addEventListeners("#showOnlyQuickWhenTextSelected, #contextMenuFirstHourOfDay", "change", function (e) {
                sendMessageToBG("addChangeContextMenuItems");
            });
...
function updateCalendarRemindersStickyTop() {
    const $siteHeader = document.querySelector(".site-header");
    const stickyTop = $siteHeader ? `${$siteHeader.getBoundingClientRect().height}px` : "0px";
    document.documentElement.style.setProperty("--calendar-reminders-sticky-top", stickyTop);
}
...
const ALL_DAY_DEFAULT_REMINDERS_KEY = "allDayDefaultReminders";

function enhanceAllDayReminderLine($calendarReminder, reminder = {}) {
    $calendarReminder.classList.add("allDayCalendarReminder");
    ...
    if ($reminderMinutes) {
        $reminderMinutes.setAttribute("value", String(displayState.reminderMinutes));
    }
    ...
    const beforeTimeSelect = createBeforeAtTimeDropdown(getAllDayReminderBeforeTimeValue(reminder));

    const $deleteReminder = $calendarReminder.querySelector(".deleteReminder");
    $calendarReminder.insertBefore(beforeAtText, $deleteReminder);
    $calendarReminder.insertBefore(beforeTimeSelect, $deleteReminder);
}

async function getAllDayCalendarReminderState(calendar) {
    const cachedFeedsDetails = await storage.get("cachedFeedsDetails");
    const savedAllDayReminders = cachedFeedsDetails?.[calendar.id]?.[ALL_DAY_DEFAULT_REMINDERS_KEY];
    if (Array.isArray(savedAllDayReminders)) {
        return {
            initialized: true,
            reminders: deepClone(savedAllDayReminders)
        };
    }
    ...
    return {
        initialized: false,
        reminders: [getDefaultAllDayReminder()]
    };
}

function collectAllDayReminders($notificationSection) {
    const reminders = [];

    $notificationSection.querySelectorAll(".calendarReminder").forEach($calendarReminder => {
        const $reminderMinutes = $calendarReminder.querySelector(".reminderMinutes");
        ...
        const $lastUpdated = $calendarReminder.querySelector(".lastUpdated");
        const $beforeTime = $calendarReminder.querySelector(".reminderBeforeTime");
        ...
        const reminderMinutes = parseInt($reminderMinutes?.value) || 0;
        ...
        // Convert to all-day offset minutes. Examples:
        // 0 + 0:00 => 0, 0 + 9:00 => -540, 1 day + 0:00 => 1440.
        reminder.minutes = reminderMinutes - beforeTimeInMinutes;
        
        $lastUpdated.value = new Date();

        if ($lastUpdated?.value) {
            reminder.lastUpdated = $lastUpdated.value;
        }
...
async function saveAllDayCalendarReminders(calendarId, allDayDefaultReminders) {
    const updatedCachedFeedsDetails = await storage.get("cachedFeedsDetails");
    if (!updatedCachedFeedsDetails[calendarId]) {
        updatedCachedFeedsDetails[calendarId] = {};
    }

    updatedCachedFeedsDetails[calendarId][ALL_DAY_DEFAULT_REMINDERS_KEY] = allDayDefaultReminders;

    await storage.set("cachedFeedsDetails", updatedCachedFeedsDetails);
    globalThis.cachedFeedsDetails = updatedCachedFeedsDetails;
}

async function removeAllDayDefaultRemindersFromCachedFeedsDetails() {
    const updatedCachedFeedsDetails = await storage.get("cachedFeedsDetails");
    let removedCount = 0;

    Object.keys(updatedCachedFeedsDetails).forEach(key => {
        if (updatedCachedFeedsDetails[key] && Object.hasOwn(updatedCachedFeedsDetails[key], ALL_DAY_DEFAULT_REMINDERS_KEY)) {
            delete updatedCachedFeedsDetails[key][ALL_DAY_DEFAULT_REMINDERS_KEY];
            removedCount++;
        }
    });
...
async function saveCalendarReminders(node) {
    ...
    // save all day immediately
    const $notificationSection = node.closest(".notification-section");
    const isAllDayNotificationSection = $notificationSection?.classList.contains("all-day-notification-section");

    if (isAllDayNotificationSection) {
        const allDayDefaultReminders = collectAllDayReminders($notificationSection);
        const calendarId = node.closest("[calendar-id]").getAttribute("calendar-id");

        await saveAllDayCalendarReminders(calendarId, allDayDefaultReminders);
        return;
    }

    clearTimeout(saveCalendarRemindersTimeout);

    saveCalendarRemindersTimeout = setTimeout(async function() {
...
function initCalendarReminders() {
	selector("#calendar-reminders").querySelectorAll(".calendarReminder").forEach($calendarReminder => {
        ...
        const isAllDayReminder = Boolean($calendarReminder.closest(".all-day-notification-section"));

        initReminderPeriod($reminderValuePerPeriod, $reminderPeriod, $reminderMinutes, isAllDayReminder, isAllDayReminder);
        ...
        function attachListener($node, action) {
            ...
            replaceEventListeners($node, action, async function(e) {
                ...
                } else if (await hasCalendarReadWritePermissions()) {
                    saveCalendarReminders($calendarReminder);
                } else {
                    this.value = e.target.dataset.previousValue;
                }
        ...
        attachListener($reminderMethod, "change");
        attachListener($reminderMinutes, "change");
        attachListener($reminderValuePerPeriod, "input");
        attachListener($reminderPeriod, "change");
        attachListener($calendarReminder.querySelector(".reminderBeforeTime"), "change");

        onClickReplace($deleteReminder, async function() {
            if (isAllDayReminder) {
                const reminderLine = this.closest(".calendarReminder");
                const notificationSection = reminderLine.closest(".notification-section");
                const calendarId = reminderLine.closest("[calendar-id]").getAttribute("calendar-id");

                slideUp(reminderLine).then(async () => {
                    reminderLine.remove();
                    const allDayDefaultReminders = collectAllDayReminders(notificationSection);
                    await saveAllDayCalendarReminders(calendarId, allDayDefaultReminders);
                });
...
async function loadCalendarReminders() {
    ...
    for (const calendar of calendars) {
        ...
        notificationSection.setAttribute("class", "notification-section timed-notification-section layout vertical start");
        ...
        const allDayNotificationSection = document.createElement("div");
        allDayNotificationSection.setAttribute("class", "notification-section all-day-notification-section layout vertical start");
        if (calendar.id == TASKS_CALENDAR_OBJECT.id || calendar.id == CommonCalendarIds.BIRTHDAYS) {
            allDayNotificationSection.hidden = true;
        }

        const allDayReminderState = await getAllDayCalendarReminderState(calendar);
        allDayNotificationSection.dataset.initialized = String(allDayReminderState.initialized);
        const allDayReminderFragment = generateReminderSection(allDayReminderState.reminders);

        allDayReminderFragment.querySelectorAll(".calendarReminder").forEach(($calendarReminder, index) => {
            const reminder = allDayReminderState.reminders[index] || {};
            enhanceAllDayReminderLine($calendarReminder, reminder);
        });

        allDayReminderFragment.querySelector(".addReminder").addEventListener("click", async function() {
            const allDaySection = this.closest(".all-day-notification-section");
            const isInitialized = allDaySection.dataset.initialized == "true";
            const checkbox = calendarReminderLine.querySelector(".calendarReminderLineCheckbox");
            if (!checkbox.checked) {
                checkbox.checked = true;
                checkbox.dispatchEvent(new Event("change"));
            ...
            if (!isInitialized) {
                const currentAllDayReminders = allDaySection.querySelectorAll(".calendarReminder");
                if (!currentAllDayReminders.length && calendar.defaultReminders?.length) {
                    const defaultAllDayReminder = getDefaultAllDayReminder();
                    const defaultNotifLine = generateReminderLine(defaultAllDayReminder);
                    enhanceAllDayReminderLine(defaultNotifLine, defaultAllDayReminder);
                    this.parentElement.insertBefore(defaultNotifLine, this);
                }
            }
            ...
            const newNotifLine = generateReminderLine(allDayReminder);
            enhanceAllDayReminderLine(newNotifLine, allDayReminder);
            this.parentElement.insertBefore(newNotifLine, this);

            allDaySection.dataset.initialized = "true";

            initCalendarReminders();
            saveCalendarReminders(newNotifLine);
        });

        allDayNotificationSection.append(allDayReminderFragment);
        calendarReminderLine.appendChild(allDayNotificationSection);
...
(async () => {
    ...
    await initUI();
    updateCalendarRemindersStickyTop();

    window.addEventListener("resize", updateCalendarRemindersStickyTop);

    // patch for firefox select > option tag doesn't support embedded markup
    if (DetectClient.isFirefox()) {
        const processedOptions = new Set();

        const processOptions = (root) => {
            root.querySelectorAll("option").forEach(option => {
                ...
                if (option.hasAttribute("msgFF")) {
                    option.textContent = getMessage(option.getAttribute("msgFF"));
                }
            });
        }

        selectorAll("template").forEach(template => {
            processOptions(template.content);
        });
