...
function setSafeHTML(element, htmlString) {
    ...
    //const containsImagePlaceholders = new RegExp(`<meta[^>]*\\b${IMAGE_PLACEHOLDER_ATTR}\\b`, "i").test(safeString);

    const jdomTemplateSanitizer = new Sanitizer({
        elements: [
            // Core HTML used in jdom templates
            "div", "span", "button", "label", "input", "textarea", "img",
            "select", "option", "menu", "dialog", "slot", "a", "br", "p", "em", "small", "strong", "blockquote",
            "h1", "h2", "h3", "h4", "h5", "h6",
            "ol", "ul", "li",
            ...
            "meta", // for image placeholders

            // Gmail html specific
            "font", "center",

            // SVG used by spinner/icons
            //"svg", "circle", "path", "defs", "g",
        ],
        attributes: [
            "id", "class", "style", "part", "role", "title",
            ...
            // table
            "valign", "colspan", "rowspan", "align", "scope",

            "dummy-jattribute", // for image placeholders

            // SVG attrs used in spinner/icon templates
            /*
            "viewBox", "preserveAspectRatio", "aria-hidden",
            "cx", "cy", "r", "fill", "stroke", "stroke-width",
            "stroke-linecap", "stroke-dasharray", "transform"
            */
        ],
    });

    element.setHTML(safeString, {
        sanitizer: jdomTemplateSanitizer
    });
}
	...
	$nodes.forEach(async element => {
		...
		if (key && key != "language") { // ignore lang because we use a specific logic inside the options.js
            if (element.nodeName.equalsIgnoreCase("input")) {
                ...
                element.value = storageValue ?? "";
            } else if (element.nodeName.equalsIgnoreCase("j-switch")) {
                element.checked = toBool(storageValue);
            }
		} else if (permissions) {
            try {
	...
	nodes.forEach(el => {
		...
		if (attr) {
			...
			if (msgArg1) {
				...
				if (innerMsg.length) {
                    ...
                    setSafeHTML(el, getMessage(attr, msgArgs));
				} else {
		...
		if (attr) {
			...
			if (msgArg1) {
                ...
                setSafeHTML(el, getMessage(el.getAttribute("msgHTML"), args));
			} else {
				// look for inner msg nodes to replace before...
                const innerMsg = el.querySelectorAll("*[msg]");
				if (innerMsg.length) {
                    ...
                    setSafeHTML(el, getMessage(attr));
				}
			}
		}
...
function ChromeTTS() {
	...
	function play() {
        ...
        if (!chrome.tts) {
            return Promise.reject(new Error("Chrome TTS not available"));
        }
...
function getPreferredLanguage() {
    ...
    // patch for Firefox users with returning this: chrome://global/locale/intl.properties
    if (lang.includes("chrome://") || !lang) {
...
function parseRRuleForByDay(byday) {
    ...
    // Parse by token so negative ordinals like "-1FR" are preserved.
    const tokenRegex = new RegExp(`^(-?[1-9])(${dayPattern})$`);
    const matchingToken = byday?.split(',').find(token => tokenRegex.test(token.trim()));
    return matchingToken?.trim().match(tokenRegex) || null;
}
...
function getUrls(str) {
    const matches = str?.match(/(\b(https?|ftp):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gi);
    return matches || [];
}