...
var notifications;
let lastSnoozeMoreDateTime;

var email;
let calendarMap;
var skinsSettings;

const REMINDER_ERROR_LOG_KEY = "_reminderConsoleErrorLogs";
const REMINDER_ERROR_LOG_MAX = 10;

function serializeReminderErrorArg(arg) {
    if (arg instanceof Error) {
        const errorCause = arg["cause"];
        return {
            name: arg.name,
            message: arg.message,
            stack: arg.stack,
...
async function persistReminderConsoleError(args) {
    try {
        const entry = {
            time: new Date().toISOString(),
            args: args.map(serializeReminderErrorArg)
        };

        let logs = await storage.get(REMINDER_ERROR_LOG_KEY);
        if (!Array.isArray(logs)) {
            logs = [];
        }

        logs.push(entry);
        logs = logs.slice(-REMINDER_ERROR_LOG_MAX);

        await storage.set(REMINDER_ERROR_LOG_KEY, logs);
    } catch (e) {
        // Avoid recursive console logging if storage is unavailable.
    }
}

function initReminderConsoleErrorLogger() {
    if (globalThis._reminderConsoleErrorLoggerInitialized) {
        return;
    }
    globalThis._reminderConsoleErrorLoggerInitialized = true;

    const originalConsoleError = console.error.bind(console);
    console.error = function(...args) {
        originalConsoleError(...args);
        persistReminderConsoleError(args);
    };
}

initReminderConsoleErrorLogger();
                    ...
                    if (hadVerticalScrollBar) {
                        ...
                        chrome.windows.getCurrent(thisWindow => {
                            ...
                            chrome.windows.update(thisWindow.id, windowParams).catch(async error => {
                                console.error("error updating window size", error, new Date(), thisWindow, thisWindow.top, thisWindow.height, resizeHeight + 1);
                                const message = String(error?.message || error);
                                if (/invalid value for bounds/i.test(message)) {

                                    if (inLocalExtension) {
                                        showError("dev only: error updating window size, retrying: " + message);
                                    }
...
async function updateTimeElapsed() {
	selectorAll(".event").forEach(async $event => {
		...
		if (timeElapsedMsg) {
            ...
            try {
                const response = await generateReminderTimes(notification.event);
    ...
    selectorAll("j-button[snoozeInDays]").forEach(async el => {
        ...
        const formatSnoozeTarget = (date) => {
            ...
            if (isTomorrow(date)) {
                return `${getMessage("tomorrow")} @ ${localizedTime}`;
            } else {
                return `${weekday} @ ${localizedTime}`;
            }
        };

        const sameTimeSnoozeDate = getSameTimeSnoozeDate(snoozeInDays);
        const isSameTimeSnooze = !!sameTimeSnoozeDate;
        let snoozeDate;
        if (isSameTimeSnooze) {
            snoozeDate = sameTimeSnoozeDate;
        } else {
            snoozeDate = new Date().addDays(snoozeInDays);
        }

        let tooltipStr;
        if (isSameTimeSnooze) {
            tooltipStr = formatSnoozeTarget(snoozeDate);
        } else if (snoozeInDays == "1.5") {
            const noon = new Date(snoozeDate);
            noon.setHours(12, 0, 0, 0);
            const localizedNoon = noon.toLocaleTimeString(locale, {
                hour: "numeric",
                minute: "2-digit"
            });
            tooltipStr = `${getMessage("tomorrow")} @ ${localizedNoon}`;
        } else if (snoozeInDays == "7") {
        ...
        if (thisWindow.state == "normal") {
            ...
            globalThis.lastDisplayChangedTimeout = setTimeout(async () => {
                ...
                if (thisWindow.state == "normal") {
                    ...
                    try {
                        console.error("resizing window in onDisplayChanged", reminderWindowParams, new Date(), thisWindow);
                        await chrome.windows.update(thisWindow.id, {
                            width: reminderWindowParams.width,
                            height: reminderWindowParams.height
                        });
                    } catch (error) {
                        console.error("error updating window size", error, new Date(), thisWindow);
                    }
                }
            }, seconds(4));
        }
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
    (async function() {
        if (message.action == "dismissAll") {
            ...
            console.error("remove notifications from reminder window", message.notifications);
...
function formatLastSnoozeMoreDateTimeLabel(date) {
    const includeTime = !!(date.getHours() || date.getMinutes() || date.getSeconds() || date.getMilliseconds());

    if (isToday(date)) {
        return date.toLocaleTimeStringJ();
    }

    if (isTomorrow(date)) {
        if (includeTime) {
            return `${getMessage("tomorrow")} ${date.toLocaleTimeStringJ()}`;
        } else {
            return getMessage("tomorrow");
        }
    }

    if (includeTime) {
        return `${date.toLocaleDateStringJ()} ${date.toLocaleTimeStringJ()}`;
    } else {
        return date.toLocaleDateStringJ();
    }
}
...
(async () => {
    ...
    // had to move this after initUI because DetectClient uses async to initialize platform
    ...
    */
    ...
    if (await shouldShowReducedDonationMsg(true)) {
        ...
        Controller.getMinimumPayment().then(minPaymentObj => {
            ...
            const reducedDonationPopupLine2 = getMessage("reducedDonationAd_popup_line2", [getMessage("extraFeatures"), formatCurrency(minPaymentObj.onetime_payment_reduced)]);
    ...
    //insertScript("js/custom-icons.js", "custom-icons").then(async () => {
        ...
        onClick(".more", async function(event) {
            if (await storage.get("donationClicked")) {
                ...
                if (lastSnoozeMoreDateTime) {
                    const lastSnoozeWrapper = document.createElement("div");
                    lastSnoozeWrapper.style.marginTop = "10px";
                    lastSnoozeWrapper.style.display = "flex";
                    lastSnoozeWrapper.style.justifyContent = "center";

                    const lastSnoozeButton = document.createElement("j-button");
                    lastSnoozeButton.id = "date-time-snooze-last-button";
                    lastSnoozeButton.classList.add("filled");
                    lastSnoozeButton.setAttribute("icon", "redo");
                    lastSnoozeButton.textContent = `${formatLastSnoozeMoreDateTimeLabel(lastSnoozeMoreDateTime)}`;
                    onClick(lastSnoozeButton, async () => {
                        await snoozeAndClose({
                            $event: globalThis.dateTimeSnoozeEvent,
                            snoozeTime: new Date(lastSnoozeMoreDateTime)
                        });

                        sendGA("reminders", "moreLastSnoozeDateTime");

                        byId("dateTimeSnoozeDialog").close();
                    });
                    lastSnoozeWrapper.append(lastSnoozeButton);
                    dialogContent.append(lastSnoozeWrapper);
                }
                ...
                openDialog(dialogContent, {
                    ...
                    onClick: async (dialog) => {
                        if (await donationClicked("snoozeDateTime")) {
                            ...
                            lastSnoozeMoreDateTime = new Date(snoozeTime);
                ...
                const defaultHourForTimedEvent = await getDefaultHourForTimedEvent();
                    ...
                    changeDate: function() {
                        if (!this.dateTime.isToday()) {
                            globalThis.snoozeTimePicker = new TimePicker(byId("timeSnooze"), {
                                defaultHour: defaultHourForTimedEvent,
                            });
                        }
                    }                
                });
