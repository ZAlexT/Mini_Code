...
const MULTI_MONTH_DURATION = { months: 4 };
...
function getFCViewName(viewName) {
    ...
    } else if (viewName == CalendarView.MONTH_SCROLL) {
        fcViewName = "multiMonth";
...
function getFCYearViewScroller() {
    return byId("betaCalendar").querySelector(".fc-view.fc-scroller");
}
...
const MAX_WEEKS_BEFORE_REMINDER = 4;
...
const STORAGE_DEFAULTS = {
    ...
    "showOnlyQuickWhenTextSelected": false,
    "contextMenuFirstHourOfDay": 7,
    "defaultHourForTimedEvent": 8,
};
...
const TASKS_CALENDAR_OBJECT = {
    ...
    defaultReminders: [getDefaultAllDayReminder()]
}
...
async function getDefaultHourForTimedEvent() {
	const hour = await storage.get("defaultHourForTimedEvent");
...
async function generateGoogleCalendarEvent(eventEntry) {
    ...
    if (!eventEntry.allDay &&eventEntry.timeZone) {
	...
	if (eventEntry.reminders?.useDefault) {
		// nothing
	} else {
...
function hasExplicitQuickAddTime(text) {
    return /\b\d{1,2}(?:[:\.]\d{2})?\s*(?:a|p)\.?m?\.?\b/i.test(text)
        || /\b\d{1,2}:\d{2}\b/.test(text)
        || /\b\d{1,2} ?h ?\d{2}\b/i.test(text);
}

function getDateOrderForLocale(localeToUse) {
    const referenceDate = new Date(2001, 10, 22); // 22 Nov 2001 to avoid ambiguous values.
    const normalizedLocale = (localeToUse || "en").replace("_", "-");
    let formatter;
...
function parseSlashDateByLocale(text, localeToUse) {
    const slashDateRegex = /\b(\d{1,2})\/(\d{1,2})\/(\d{2,4})\b/;
    const matches = slashDateRegex.exec(text);
    if (!matches) {
        return;
    }
    ...
    const dateOrder = getDateOrderForLocale(localeToUse);
    const isDayFirst = dateOrder.startsWith("daymonth");
    ...
    return {
        date: parsedDate,
        text: text.replace(matches[0], "").replace(/\s{2,}/g, " ").trim()
    };
}
...
async function getEventEntryFromQuickAddText(text) {
    ...
    let timeStr = "";
    let newDate;
    const explicitTimeSubmitted = hasExplicitQuickAddTime(text);
    const language = await storage.get("language");
    const slashDateFilter = parseSlashDateByLocale(text, language);
    if (slashDateFilter) {
        text = slashDateFilter.text;
        newDate = slashDateFilter.date;
    }

    if (!explicitTimeSubmitted) {
...
function updateEventShown(event, eventsShown) {
	...
	if (eventShown) {
		markReminderTimeAsShown(event, eventShown);
	} else {
...
function markReminderTimeAsShown(event, eventShown) {
...
function getEventReminders(event) {
	let reminders;
    const eventCalendar = getEventCalendar(event);

    function getAllDayDefaultReminders(calendar) {
        const allDayDefaultReminders = globalThis.cachedFeedsDetails[calendar.id]?.allDayDefaultReminders;
        if (allDayDefaultReminders) {
            return allDayDefaultReminders.map(reminder => ({
                method: reminder.method,
                minutes: parseInt(reminder.minutes)
            }));
        }

        if (calendar.defaultReminders?.length) {
            return [getDefaultAllDayReminder()];
        }
	...
	if (event.reminders) {
		if (event.reminders.useDefault) {
            if (event.allDay) {
                reminders = getAllDayDefaultReminders(eventCalendar);
            } else {
                reminders = eventCalendar.defaultReminders;
            }
		} else {
            if (event.allDay && (!event.reminders.overrides || event.extendedProperties?.private?.useDefaultAllDayReminders)) {
                reminders = getAllDayDefaultReminders(eventCalendar);
            } else {
                reminders = event.reminders.overrides;

                if (event.allDay) {
                    // Keep one canonical 0-day reminder from defaults and drop 0-day overrides.
                    const allDayDefaultReminders = getAllDayDefaultReminders(eventCalendar);
                    const preferredZeroDayReminder = allDayDefaultReminders.find(reminder => {
                        return getAllDayReminderDisplayState(reminder).reminderMinutes == 0;
                    });
                    ...
                    const mergedReminders = (reminders || []).map(reminder => ({
                        ...
                        if (!preferredZeroDayReminder) {
                            return true;
                        }

                        const displayState = getAllDayReminderDisplayState(reminder);
                        return displayState.reminderMinutes != 0;
                    });
                    const mergedReminderKeys = new Set(mergedReminders.map(reminder => `${reminder.method}:${reminder.minutes}`));

                    if (preferredZeroDayReminder) {
                        const reminderMinutes = parseInt(preferredZeroDayReminder.minutes);
                        const reminderKey = `${preferredZeroDayReminder.method}:${reminderMinutes}`;
                        if (!mergedReminderKeys.has(reminderKey)) {
                            mergedReminderKeys.add(reminderKey);
                            mergedReminders.push({
                                method: preferredZeroDayReminder.method,
...
const DEFAULT_EVENT_NOTIFICATION_TIME = "9:00";

function getDefaultEventNotificationTime() {
    return DEFAULT_EVENT_NOTIFICATION_TIME.parseTime();
}

function getDefaultAllDayReminder() {
    return {
        method: "popup",
        minutes: 0,
    };
...
function getDefaultAllDayReminderAdd() {
    ...
    return {
        method: "popup",
        minutes: DAY_IN_MINUTES - beforeTimeInMinutes,
    }
}

function getDefaultTimedReminderAdd() {
    return {
        method: "popup",
        minutes: 10,
    }
...
function normalizeReminderTimeValue(timeValue) {
    const timeMatch = timeValue.trim().match(/^(-?\d{1,3}):(\d{1,2})$/);
    const hour = parseInt(timeMatch[1]);
    const minute = parseInt(timeMatch[2]);
    const totalMinutes = (hour * HOUR_IN_MINUTES) + minute;
    const normalizedMinutes = ((totalMinutes % DAY_IN_MINUTES) + DAY_IN_MINUTES) % DAY_IN_MINUTES;
    const normalizedHour = Math.floor(normalizedMinutes / HOUR_IN_MINUTES);
    const normalizedMinute = normalizedMinutes % HOUR_IN_MINUTES;

    return `${normalizedHour}:${normalizedMinute.toString().padStart(2, "0")}`;
}

function minutesToTimeValue(minutes) {
    const parsedMinutes = parseInt(minutes) || 0;
    const safeMinutes = ((parsedMinutes % DAY_IN_MINUTES) + DAY_IN_MINUTES) % DAY_IN_MINUTES;
    const hours = Math.floor(safeMinutes / HOUR_IN_MINUTES);
    const mins = safeMinutes % HOUR_IN_MINUTES;
    return `${hours}:${mins.toString().padStart(2, "0")}`;
}

function getSameTimeSnoozeDays(daysToSnooze) {
    if (typeof daysToSnooze !== "string" || !daysToSnooze.includes("_sametime")) {
        return null;
    }

    const days = parseInt(daysToSnooze.split("_")[0], 10);
    return Number.isNaN(days) ? null : days;
}

function getSameTimeSnoozeDate(daysToSnooze, baseDate = new Date()) {
    const sameTimeSnoozeDays = getSameTimeSnoozeDays(daysToSnooze);
    if (sameTimeSnoozeDays == null) {
        return null;
    }

    const snoozeDate = new Date(baseDate);
    snoozeDate.setDate(snoozeDate.getDate() + sameTimeSnoozeDays);
    return snoozeDate;
}

function getWholeDaySnoozeTime(notification) {
    let beforeTimeMinutes = getAllDayReminderDisplayState(getDefaultAllDayReminder().minutes).beforeTimeMinutes;

    const eventCalendar = getEventCalendar(notification?.event || {});
    const allDayDefaultReminders = eventCalendar?.id
        ? globalThis.cachedFeedsDetails?.[eventCalendar.id]?.allDayDefaultReminders
        : null;

    if (Array.isArray(allDayDefaultReminders)) {
        const zeroDayReminder = allDayDefaultReminders.find(reminder => {
            return getAllDayReminderDisplayState(reminder).reminderMinutes == 0;
        });

        if (zeroDayReminder) {
            beforeTimeMinutes = getAllDayReminderDisplayState(zeroDayReminder).beforeTimeMinutes;
        }
    }

    return {
        hours: Math.floor(beforeTimeMinutes / HOUR_IN_MINUTES),
        minutes: beforeTimeMinutes % HOUR_IN_MINUTES
    };
}
...
async function snoozeNotifications(snoozeParams, notifications) {
	if (snoozeParams.snoozeTime) {
        ...
        // Detect if "same time" snooze (24h, 48h from current time)
        const snoozeFromNow = getSameTimeSnoozeDate(daysToSnooze);
        if (snoozeFromNow) {
            wholeDays = false;
            snoozeToThisDay = snoozeFromNow;
        } else if (daysToSnooze == 1.5) {
            wholeDays = false;
            ...
            const wholeDaySnoozeTime = getWholeDaySnoozeTime(notifications?.[0]);
            snoozeToThisDay.setHours(wholeDaySnoozeTime.hours, wholeDaySnoozeTime.minutes, 0, 0);
        }
			...
			if (urls.length) {
				...
				if (link) {
					...
					if (link.match("https?://mail.google.com")) {
                        ...
                        return null;
                    } else if (link.match("https?://tasks.google.com/task")) {
                        // Ignore event generated from timed tasks, cause description says "Changes made to the title, description, or attachments will not be saved. To make edits, please go to: https://tasks.google.com/task/XkhDeuy4LY0XOXt4"
                        return null;
					} else {
...
function isMeetingLink(url) {
    if (url) {
        ...
        return meetingDomains.some(domain => new RegExp(`[^a-z]${domain.replace(/\./g, "\\.")}`, "i").test(url));
    }
}
...
function getMeetingLink(str) {
    ...
    const url = urls?.find(url => isMeetingLink(url));
    if (url) {
...
function isColorTooLight(color, luminosity) {
	...
	if (rgb) {
        ...
        if (luminosity) {
            ...
            // May 2026 changed from .85 to .62 cause yellow calendar is very light and hard to see white text
            if (l >= 0.62 || isYellow) {
                return true;
            }
        }
...
function getEventColors(params) { // event, darkenColorFlag, cachedFeeds, arrayOfCalendars
	...
	if (params.darkenColorFlag) {
        ...
        // if dark theme use black else white
		if (params.darkMode == "dark" || (htmlElement?.getAttribute("color-scheme") == "dark")) {
...
function initReminderPeriod($reminderValuePerPeriod, $reminderPeriod, $reminderMinutes, allDay, patchForAllDayBeforeXTime) {
    ...
    const obj = getUserFriendlyReminderTime(reminderMinutes, allDay, patchForAllDayBeforeXTime);
    $reminderValuePerPeriod.value = obj.value;
    $reminderPeriod.value = obj.period;

    const reminderValuePerPeriodMax = $reminderPeriod.value == "weeks" ? MAX_WEEKS_BEFORE_REMINDER : null;
    if (reminderValuePerPeriodMax != null) {
        $reminderValuePerPeriod.max = reminderValuePerPeriodMax;
        if (parseInt($reminderValuePerPeriod.value) > reminderValuePerPeriodMax) {
            $reminderValuePerPeriod.value = reminderValuePerPeriodMax;
        }
    } else {
        $reminderValuePerPeriod.removeAttribute("max");
    }
}
...
function createBeforeAtTimeDropdown(selectedTimeValue) {
    const beforeTimeSelect = document.createElement("select");
    beforeTimeSelect.className = "reminderBeforeTime";

    for (let minutes = 0; minutes < DAY_IN_MINUTES; minutes += 30) {
        const option = document.createElement("option");
        const value = minutesToTimeValue(minutes);
        option.value = value;
        option.textContent = value.parseTime().toLocaleTimeStringJ();
        beforeTimeSelect.append(option);
    }

    const normalizedTimeValue = normalizeReminderTimeValue(selectedTimeValue || DEFAULT_EVENT_NOTIFICATION_TIME);
    beforeTimeSelect.value = normalizedTimeValue;

    // Fallback in case localized option values differ from expected 24h format.
    if (beforeTimeSelect.value != normalizedTimeValue && beforeTimeSelect.options.length) {
        beforeTimeSelect.value = beforeTimeSelect.options[0].value;
    }

    return beforeTimeSelect;
}

function getAllDayReminderDisplayState(reminderOrMinutes) {
    const storedMinutesRaw = typeof reminderOrMinutes == "object"
        ? reminderOrMinutes?.minutes
        : reminderOrMinutes;
    const storedMinutes = parseInt(storedMinutesRaw) || 0;

    if (storedMinutes < 0) {
        return {
            reminderMinutes: 0,
            beforeTimeMinutes: Math.min(DAY_IN_MINUTES - 1, Math.abs(storedMinutes))
        };
    }
    ...
    const dayBlocks = Math.ceil(storedMinutes / DAY_IN_MINUTES);
    const reminderMinutes = dayBlocks * DAY_IN_MINUTES;

    return {
        reminderMinutes: reminderMinutes,
        beforeTimeMinutes: reminderMinutes - storedMinutes
    };
}

function getAllDayReminderBeforeTimeValue(reminderOrMinutes) {
    const displayState = getAllDayReminderDisplayState(reminderOrMinutes);
    return minutesToTimeValue(displayState.beforeTimeMinutes);
}
...
function isCalendarWriteable(calendar) {
    return calendar.accessRole == CalendarAccessRole.OWNER
        || calendar.accessRole == CalendarAccessRole.WRITER
        || calendar.accessRole == CalendarAccessRole.WRITER_WITHOUT_PRIVATE_ACCESS;
}
...
async function formatEventAddedMessage(title, eventEntry) {
	...
	if (eventEntry.startTime) {
        if (!eventEntry.allDay) {
            const HOURS_BEFORE = 4; // only show relative time message if event is within 4 hours because
            const diffInMinutes = eventEntry.startTime.diffInMinutes();
            const roundedDiffInMinutes = Math.round(diffInMinutes);
            if (roundedDiffInMinutes > 0 && roundedDiffInMinutes < HOURS_BEFORE * 60) {
                const formatter = new Intl.RelativeTimeFormat(locale, {
                    numeric: "auto"
                });

                if (roundedDiffInMinutes >= 60) {
                    const hours = Math.floor(roundedDiffInMinutes / 60);
                    const remainingMinutes = roundedDiffInMinutes % 60;
                    const hourMessage = formatter.format(hours, "hour");
                    let relativeTimeMessage;

                    if (remainingMinutes) {
                        const minuteMessage = getMessage(remainingMinutes == 1 ? "Xminute" : "Xminutes", [remainingMinutes]);
                        relativeTimeMessage = `${hourMessage} ${minuteMessage}`;
                    } else {
                        relativeTimeMessage = hourMessage;
                    }

                    message = getMessage("addedForTimeFromNow", [title, relativeTimeMessage]);
                } else {
                    message = getMessage("addedForTimeFromNow", [title, formatter.format(roundedDiffInMinutes, "minute")]);
                }
                return message;
            }
        }
...
async function requestPermission(params = {}) {
    ...
    if (browserAutomaticallyClosesPopup() && location.href.includes("popup.html") && globalThis.fromToolbar && !isRequestingPermission) {
    ...
    try {
        ...
        const authPageLoadFailed = (error?.message || "").toLowerCase().includes("authorization page could not be loaded");

        if (authPageLoadFailed) {
            await niceAlert("Authorization page could not be loaded. This can happen if access is blocked by your organization or security policy.");
        } else if (error.cause == ErrorCause.ACCESS_DENIED) {
	...
	if (calendarColors) {
		...
		for (const key in calendarColors.calendar) {
            ...
            [color-id="${key}"].jdom-checkbox:not(:checked) {
                appearance: none;
                border: 1px solid ${checkboxColor} !important;
            }
...
async function generateReminderTimes(event) {
    const reminderTimes = [];
		...
		reminders.forEach(reminder => {
			if (reminder.method == "popup" || reminder.method == "email") {
                ...
                if (event.allDay && reminder.minutes == 0) {
                    reminderTimes.push(getMessage("onTheSameDayAt", getBeforeTimeLabel(reminder.minutes)));
                } else if (event.allDay && reminder.minutes < 0) {
                    reminderTimes.push(getMessage("onTheSameDayAt", getBeforeTimeLabel(-reminder.minutes)));
                } else if (!event.allDay && reminder.minutes == 0) {
                    ...
                    if (event.allDay && obj.beforeTime) {
                        ...
                        beforeToDisplay = getMessage("beforeStart");
                    } else {
                        beforeToDisplay = getMessage("before");
                    }
...
function getReminderPresetValues(allDay, presetReminders) {
    if (allDay) {
        const presetValues = [];

        presetReminders.forEach(reminder => {
            presetValues.push(parseInt(reminder?.minutes));
        });

        return [...new Set(presetValues)].sort((a, b) => a - b);
    }

    return [0, 5, 10, 15, 30, 60, 120, 240, 1440];
}

function getBeforeTimeLabel(beforeTimeInMinutes) {
    const date = new DateZeroTime();
    date.setMinutes(beforeTimeInMinutes);
    return date.toLocaleTimeStringJ(true);
}

function getReminderPresetLabel(minutes, allDay, reminderMethod) {
    const suffix = reminderMethod == "email" ? ` (${getMessage("email").toLowerCase()})` : "";
    let beforeTimeInMinutes;

    if (allDay) {
        const userFriendlyReminderTime = getUserFriendlyReminderTime(minutes, allDay, true);
        if (userFriendlyReminderTime.beforeTime != undefined) {
            beforeTimeInMinutes = userFriendlyReminderTime.beforeTime;
        } else if (minutes >= 0) {
            // For exact day/week multiples (ex: 1440), derive the actual reminder time
            // instead of falling back to default notification time.
            const mod = minutes % DAY_IN_MINUTES;
            beforeTimeInMinutes = (DAY_IN_MINUTES - mod) % DAY_IN_MINUTES;
        }
    }

    if (allDay && beforeTimeInMinutes == undefined) {
        const parsedDefaultEventNotificationTime = getDefaultEventNotificationTime();
        beforeTimeInMinutes = (parsedDefaultEventNotificationTime.getHours() * HOUR_IN_MINUTES) + parsedDefaultEventNotificationTime.getMinutes();
    }

    if (minutes <= 0 && allDay) {
        return `${getMessage("onTheSameDayAt", getBeforeTimeLabel(-minutes))}${suffix}`;
    }

    const obj = getUserFriendlyReminderTime(minutes, allDay, true);
    const periodName = obj.value == 1 ? obj.period.replace(/s$/, "") : obj.period;
    if (allDay) {
        return `${getMessage("X" + periodName, obj.value)} ${getMessage("beforeStart")} ${getBeforeTimeLabel(beforeTimeInMinutes)}${suffix}`;
    } else {
        if (obj.period == "minutes" && obj.value == 0) {
            return `${getMessage("whenEventStarts")}${suffix}`;
        } else {
            return `${getMessage("X" + periodName, obj.value)} ${getMessage("before")}${suffix}`;
        }
    }
}

function generateCreateEventReminderLine(reminder, allDay, presetReminders) {
    const reminderNode = document.createElement("div");
    reminderNode.classList.add("calendarReminder", "createEventReminder");

    const reminderMinutes = parseInt(reminder.minutes);

    function getPresetOptionMethod(minutes) {
        if (!allDay) {
            return reminder.method == "email" && minutes == reminderMinutes ? "email" : "popup";
        }

        const matchingPresetReminders = (presetReminders || []).filter(presetReminder => parseInt(presetReminder?.minutes) == minutes);
        if (!matchingPresetReminders.length) {
            return "popup";
        }

        // Keep baseline popup labels when both popup and email exist for the same minute.
        const popupReminder = matchingPresetReminders.find(presetReminder => presetReminder.method == "popup");
        if (popupReminder) {
            return "popup";
        }

        return matchingPresetReminders[0].method || "popup";
    }

    const presetValues = getReminderPresetValues(allDay, presetReminders);
    const presetValue = allDay
        ? (reminder.method == "popup" && presetValues.includes(reminderMinutes) ? reminderMinutes : "custom")
        : (presetValues.includes(reminderMinutes) ? reminderMinutes : "custom");

    reminderNode._presetReminders = presetReminders;

    reminderNode.innerHTML = `
        <select class="reminderPreset"></select>
        <j-button class="deleteReminder" icon="close"></j-button>
        <j-input class="reminderMethod" hidden value="${reminder.method}"></j-input>
        <j-input class="reminderMinutes" hidden value="${reminder.minutes}"></j-input>
        <j-input class="lastUpdated" hidden value="${reminder.lastUpdated || ''}"></j-input>
    `;

    const $reminderPreset = reminderNode.querySelector(".reminderPreset");
    presetValues.forEach(minutes => {
        const option = document.createElement("option");
        const presetMethod = getPresetOptionMethod(minutes);
        option.value = minutes;
        option.dataset.method = presetMethod;
        option.textContent = getReminderPresetLabel(minutes, allDay, presetMethod);
    ...
    $reminderPreset.appendChild(customOption);

    $reminderPreset.value = String(presetValue);
...
function generateCreateEventReminderSection(reminders, allDay, presetReminders) {
    const fragment = document.createDocumentFragment();

    fragment.append(generateCreateEventReminderStyles());

    reminders.forEach(reminder => {
        fragment.append(generateCreateEventReminderLine(reminder, allDay, presetReminders));
    });
...
function generateCreateEventReminderStyles() {
    const style = document.createElement("style");
    style.innerHTML = `
        .createEventReminder {
            display: flex;
        ...
        .createEventReminder .reminderPreset {
            min-width: 180px;
            flex: 1 1 180px;
        }
        .createEventReminder .deleteReminder {
            flex: 0 0 auto;
        }
        .createEventReminder .reminderMethod,
        .createEventReminder .reminderMinutes,
        .createEventReminder .lastUpdated {
            display: none;
        }
    `;
    return style;
...
function generateReminderStyles() {
    const style = document.createElement("style");
    style.innerHTML = `
        .calendarReminder {
            display: flex;
...
function generateReminderLine(reminder, params = {}) {
    ...
    if (includeStyles) {
        reminderNode.prepend(generateReminderStyles());
    }
...
function generateReminderSection(reminders) {
    ...
    fragment.append(generateReminderStyles());

    reminders.forEach(reminder => {
        fragment.append(generateReminderLine(reminder, {includeStyles: false}));
    });
