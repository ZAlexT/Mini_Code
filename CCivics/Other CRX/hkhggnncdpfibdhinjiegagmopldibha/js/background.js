...
async function addChangeContextMenuItems() {
	...
	if (chrome.contextMenus?.removeAll) {
        ...
        const CONTEXTS = await getContextsContextMenu();
        const FIRST_HOUR_OF_DAY = await storage.get("contextMenuFirstHourOfDay");
    ...
    if (!await storage.get("loggedOut")) {
        ...
        events.forEach(event => {
            ...
            if ((showBusyEvents || calendar?.accessRole != CalendarAccessRole.FREE_BUSY)
                ...
                if (!isCalendarExcludedForNotifs(calendar, excludedCalendars) && !isGadgetCalendar(calendar)
                    && event.eventType != EventType.OUT_OF_OFFICE && event.eventType != EventType.WORKING_LOCATION) {
        ...
        if (nextEvent && await storage.get("useEventColors") && (!eventInProgress || (eventInProgress && !timeRemainingForCurrentEvent))) {
            const eventColors = getEventColors({
                ...
                darkMode: await storage.get("darkMode")
            });
...
async function filterNotifications(notifications, eventsToRemoveFromReminders) {
    ...
    function extractTaskIdFromTasksUrl(url) {
        if (!url) {
            return;
        }

        const match = String(url).match(/tasks\.google\.com\/task\/([^\/?#"'<>\s]+)/i);
        if (match?.[1]) {
            return decodeURIComponent(match[1]);
        }
    }

    function getLinkedTaskIdFromEvent(event) {
        if (!event) {
            return;
        }

        if (event.kind == TASKS_KIND) {
            return extractTaskIdFromTasksUrl(event.webViewLink);
        }

        return extractTaskIdFromTasksUrl(event.description);
    }

    const candidateTaskIds = new Set();
    notifications.forEach(notification => {
        const event = notification?.event;
        if (event?.kind == TASKS_KIND && event.allDay) {
            const taskId = getLinkedTaskIdFromEvent(event);
            if (taskId) {
    ...
    const linkedTimedTaskIds = new Set();
    if (candidateTaskIds.size) {
        events.forEach(event => {
            if (!event || event.kind == TASKS_KIND || event.allDay) {
                return;
            }

            if (event.eventType == EventType.FOCUS_TIME) {
                const taskId = getLinkedTaskIdFromEvent(event);
                if (taskId && candidateTaskIds.has(taskId)) {
                    linkedTimedTaskIds.add(taskId);
                }
            }
        });
    }
	...
	for (let a=0, notification; notification=notifications[a], a<notifications.length; a++) {
        if (notification.event.kind == TASKS_KIND && notification.event.allDay) {
            const linkedTaskId = getLinkedTaskIdFromEvent(notification.event);
            if (linkedTaskId && linkedTimedTaskIds.has(linkedTaskId)) {
                console.log("removing notification for all day task linked to timed task: " + linkedTaskId);
...
async function createTaskFromGmailExternalMessage(message) {
    const title = message?.title?.trim();
    const sourceUrl = message?.url?.trim();

    if (!title) {
        throw Error("Missing required parameter: title");
    }
    ...
    if (!tasksUserEmails?.length) {
        throw Error("Google Tasks is not authorized. Please authorize Tasks in Checker Plus for Calendar first.");
    }

    const extensionEmail = await storage.get("email");
    const userEmail = tasksUserEmails.includes(extensionEmail) ? extensionEmail : tasksUserEmails[0];

    let taskLists = cachedFeeds["taskLists"];
    if (!taskLists?.items?.length) {
        taskLists = await oauthDeviceSend({
            userEmail: userEmail,
            url: `${TASKS_BASE_URL}/users/@me/lists`
        }, oAuthForTasks);
    }

    const taskList = taskLists?.items?.first?.() || taskLists?.items?.[0];
    if (!taskList?.id) {
        throw Error("No Google Tasks list found for this account.");
    }

    const createdTask = await oauthDeviceSend({
        userEmail: userEmail,
        type: "post",
        url: `${TASKS_BASE_URL}/lists/${taskList.id}/tasks`,
        data: {
            title: title,
            notes: sourceUrl
        }
    ...
    const taskUrl = createdTask?.webViewLink || (createdTask?.id ? `https://tasks.google.com/task/${createdTask.id}` : createdTask?.selfLink);
    if (!taskUrl) {
        throw Error("Task was created but no task URL was returned.");
    }
	...
	chrome.runtime.onMessageExternal.addListener((message, sender, sendResponse) => {
        (async function() {
            ...
            if (message.action == "turnOffDND") {
                ...
                sendResponse();
            } else if (message.action == "createTask") {
                if (isGmailExtension(sender.id)) {
                    try {
                        const taskUrl = await createTaskFromGmailExternalMessage(message);
                        sendResponse({
                            success: true,
                            taskUrl: taskUrl,
                            url: taskUrl
                    ...
                    const error = "Message not sent from a recognized extension";
                    console.warn(`${error}: ` + sender.id);
                ...
                sendResponse({actionNotSupported: true});
            }
        })();
