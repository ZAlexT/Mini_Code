/* 	license2020.js - This is needed because of Chrome Web Store payments deprecation
	...
	So not I have to process license using another service such as Paypal. But to keep
...
if (!sync) 
	var sync = {};
if (typeof document === "undefined") // Version 1.5.0
	var document = {}; // Service workers don't have document so to avoid errors add it
var test_mode = ('update_url' in chrome.runtime.getManifest()) ? false : true; // Version 1.5.0 - If loaded from Web Store instead of local folder than getManifest has update_url property
...
// Version 1.5.0 - Jeff try this ajax_fetch() function in place of ajax()
...
function getLicense2020(bit) {
	...
	chrome.identity.getProfileUserInfo(null, function(userInfo) { // chrome.identity can't be run in content scripts
		...
		if (userInfo.email != "" || userInfo.id != "" || userInfo.order_id != "") {
			// Version 1.5.0 - From ajax to ajax_fetch below
		...
		/*if (userInfo.order_id != "") {
			...
			if (document.settings_form && document.settings_form.status) // Version 1.4.5
...
/*if (location.pathname == "/_generated_background_page.html")
	setTimeout(function(){ 
		...
		|| !chrome.runtime.getManifest().oauth2) // Version 1.4.2i - In case I accidentally leave license.js in sr.html for 365 edition		
			init2020(); 
	}, 2000); // Version 1.4.1 - Added setTimeout because we need license.js to run first to check for chromewebstore full license
if (document.settings_form && document.settings_form.check_order) {
	settings_form.check_order.addEventListener('click', function() { 
...
chrome.storage.sync.get('license', function(obj) { // Version 1.4.0
	//if (!sync) window['sync'] = {}; // Version 1.5.0 - Removed because sync is now defined at top
	sync.license = obj.license || {};
	if (sync.license.license == "chromewebstore#userLicense") // Version 1.3.8 - Fix 1.3.7 license = kind bug
		sync.license.license = "FULL"; 
	if (sync.license.license && sync.license.license.match(/full/i)) // Version 1.3.7f
	...
	/*if (sync.license.error && document.getElementById('error')) // Version 1.4.0
			...
			"See Help</a>";*/ // Version 1.0.3 - Removed - Only display error if it comes from ajax not from storage
			
	//if (running_script == "background")
	//if (location.pathname == "/_generated_background_page.html") {
	if (location.pathname.match(/popup/i)) { // Version 1.5.0 - Going to let popup check the license now on open
		if (typeof getLicense !== 'function' // Version 1.4.2b - If license.js is installed then it will run check2020()
		|| !chrome.runtime.getManifest().oauth2) // Version 1.4.2i - In case I accidentally leave license.js in sr.html for 365 edition		
			init2020(); 
		else
...
chrome.runtime.onMessage.addListener( function(obj, sender, sendResponse) { // Version 1.4.2b
	...
	if (obj.hasOwnProperty("getLicense")) 
		//if (location.pathname == "/_generated_background_page.html") // Only let background script run this not sr.js // Version 1.5.0 - Removed
			init2020(obj["getLicense"]); // obj["getLicense"] has order_id in it
	if (obj.hasOwnProperty("update_title")) // Version 1.4.2b
		if (typeof window[update_title_function] === "function")
			window[update_title_function](); // Call the functions to update the extension title and full license features
	if (obj.hasOwnProperty("getLicense") || obj.hasOwnProperty("update_title")) // Version 1.5.0 - Added because colors "reset" in fontsize2.js was unable to send response with default website colors
...
window.addEventListener("paddle", function(e) { 
	...
	if (test_mode && window) console.log(window.location.hostname); // Version 1.5.0 - Added && window here and !window below
	if (!window && !window.location.hostname.match(/seabreezecomputers.com/i)) return; // Don't listen if not from website
	//var products_array = [643310]; // A+ FontSize Changer product_ids
	if (e.detail.hasOwnProperty("order")) {
		...
		if (products_array.indexOf(product_id) > -1) { // Version 1.3.7g - Changed e.detail.order["product_id"] to product_id
			...
			var cd2 = new Date(); cd2.setDate(cd2.getDate()-10); // Version 1.5.0b - Subtract 10 days from cd2 so next time they restart it will check for license
			sync.license["cd2"] = cd2.getTime(); // Version 1.5.0c - Added .getTime() because it was object instead of number
			chrome.storage.sync.set({'license' : sync.license}, function() { // Version 1.3.8 - Added , function() instead of setTimeout below
				// send_to_background({"command": "init2020", "option": 1}); // Version 1.4.2b - Replaced with below
				chrome.runtime.sendMessage({"getLicense": sync.license.order_id}, function(response) { // Version 1.4.2b - Because send_to_backgroud only worked if sr.html was open // Version 1.4.4 - From obj.order_id to sync.license.order_id
...
// Version 1.5.3 - Listen for lemonsqueezy event in content script
window.addEventListener("lemon", function(e) { 
	if (test_mode) console.log(e.detail); 
	...
	var products_array = [592820, 923500, 592832, 923513]; // LS Test mode A2C product_id and variants, LS Live mode A2C product_id and variants
	if (e.detail.attributes && e.detail.attributes.first_order_item) {
		if (test_mode) console.log(JSON.stringify(e.detail));
		var product_id = e.detail.attributes.first_order_item.product_id;
		var product_name = e.detail.attributes.first_order_item.product_name;
		if (products_array.indexOf(product_id) > -1
			|| product_name.match(/Add To Calendar/i))
		{
			sync.license["order_id"] = e.detail.id; // It appears the order_id should always be used with paddle
			sync.license["license"] = "FULL";
			sync.license["kind"] = "lemon";
			sync.license["status"] = e.detail.attributes.status;
			var cd2 = new Date(); cd2.setDate(cd2.getDate()-10); // Version 1.5.0b - Subtract 10 days from cd2 so next time they restart it will check for license
			sync.license["cd2"] = cd2.getTime(); // Version 1.5.0c - Added .getTime() because it was object instead of number
			chrome.storage.sync.set({'license' : sync.license}, function() { // Version 1.3.8 - Added , function() instead of setTimeout below
				// send_to_background({"command": "init2020", "option": 1}); // Version 1.4.2b - Replaced with below
...
// Version 1.5.0c - This is also in storage.js so we don't have to load license2020.js in content all the time
chrome.storage.onChanged.addListener(function(changes, namespace) { // Version 1.5.0c - Put license changes in content.js sync
